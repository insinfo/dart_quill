...
class _Fittable {
  ...
  _Fittable(
      this.sliceDepth, this.frontierDepth, this.parent, this.inject, this.wrap);
}
...
class _Fitter {
  ...
  _Fitter(this.fromPos, this.toPos, this.unplaced) {
    for (int i = 0; i <= fromPos.depth; i++) {
      ...
      frontier.add(
          _FrontierNode(node.type, node.contentMatchAt(fromPos.indexAfter(i))));
    }
  ...
  Step? fit() {
    ...
    ResolvedPos? finalTo =
        _close(moveInline < 0 ? toPos : fromPos.doc.resolve(moveInline));
    ...
    if (moveInline > -1) {
      return ReplaceAroundStep(
          fromPos.pos, moveInline, toPos.pos, toPos.end(), slice, placedSize);
    }
  ...
  _Fittable? _findFittable() {
    ...
    for (int pass = 1; pass <= 2; pass++) {
      for (int sliceDepth = pass == 1 ? startDepth : unplaced.openStart;
          sliceDepth >= 0;
          sliceDepth--) {
        Fragment fragment;
        PMNode? parent;
          ...
          if (pass == 1 &&
              (first != null
                  ? match.matchType(first.type) != null ||
                      (inject =
                              match.fillBefore(Fragment.from(first), false)) !=
                          null
  ...
  void _dropNode() {
    ...
    if (inner.childCount <= 1 && openStart > 0) {
      ...
      unplaced =
          Slice(_dropFromFragment(content, openStart, 1), openStart, openEnd);
    }
  }
  ...
  void _placeNodes(_Fittable fittable) {
    ...
    Fragment fragment =
        fittable.parent != null ? fittable.parent!.content : slice.content;
    ...
    int openEndCount = (fragment.size + fittable.sliceDepth) -
        (slice.content.size - slice.openEnd);
    ...
    if (toEnd &&
        openEndCount < 0 &&
        fittable.parent != null &&
        fittable.parent!.type == frontier[depth].type &&
        frontier.length > 1) {
      _closeFrontierNode();
    }
    ...
    for (int i = 0; i < openEndCount; i++) {
      ...
      frontier
          .add(_FrontierNode(node.type, node.contentMatchAt(node.childCount)));
      cur = node.content;
    }

    if (!toEnd) {
      unplaced = Slice(
          _dropFromFragment(slice.content, fittable.sliceDepth, taken),
          slice.openStart,
          slice.openEnd);
      ...
      unplaced = Slice(
          _dropFromFragment(slice.content, fittable.sliceDepth - 1, 1),
          fittable.sliceDepth - 1,
          openEndCount < 0 ? slice.openEnd : fittable.sliceDepth - 1);
    }
  }
  ...
  int _mustMoveInline() {
    ...
    if (!top.type.isTextblock ||
        _contentAfterFits(toPos, toPos.depth, top.type, top.match, false) ==
            null) {
      return -1;
    }
  ...
  _CloseLevel? _findCloseLevel(ResolvedPos toPosArg) {
    for (int i = min(depth, toPosArg.depth); i >= 0; i--) {
      ...
      ContentMatch match = fNode.match;
      NodeType type = fNode.type;
      bool dropInner = i < toPosArg.depth &&
          toPosArg.end(i + 1) == toPosArg.pos + (toPosArg.depth - (i + 1));
      ...
      for (int d = i - 1; d >= 0; d--) {
        ...
        Fragment? matches =
            _contentAfterFits(toPosArg, d, dfNode.type, dfNode.match, true);
      ...
      if (valid)
        return _CloseLevel(i, fit,
            dropInner ? toPosArg.doc.resolve(toPosArg.after(i + 1)) : toPosArg);
    }
    return null;
  }
  ...
  ResolvedPos? _close(ResolvedPos toPosArg) {
    ...
    for (int d = close.depth + 1; d <= toPosArg.depth; d++) {
      ...
      Fragment add = node.type.contentMatch
          .fillBefore(node.content, true, toPosArg.index(d))!;
      _openFrontierNode(node.type, node.attrs, add);
    }
    return toPosArg;
  }

  void _openFrontierNode(NodeType type,
      [Map<String, dynamic>? attrs, Fragment? content]) {
    _FrontierNode top = frontier[depth];
    top.match = top.match.matchType(type)!;
    placed = _addToFragment(
        placed, depth, Fragment.from(type.create(attrs, content)));
...
PMNode _closeNodeStart(PMNode node, int openStart, int openEnd) {
  ...
  if (openStart > 1) {
    frag = frag.replaceChild(
        0,
        _closeNodeStart(frag.firstChild!, openStart - 1,
            frag.childCount == 1 ? openEnd - 1 : 0));
  }
  if (openStart > 0) {
    frag = node.type.contentMatch.fillBefore(frag)!.append(frag);
    if (openEnd <= 0) {
      frag = frag.append(node.type.contentMatch
          .matchFragment(frag)!
...
Fragment? _contentAfterFits(ResolvedPos toPos, int depth, NodeType type,
    ContentMatch match, bool open) {
  ...
  return (fit != null && !_invalidMarks(type, node.content, index))
      ? fit
      : null;
}
...
void replaceRange(Transform tr, int from, int to, Slice slice) {
  ...
  if (targetDepths.isNotEmpty && targetDepths.last == 0)
  ...
  for (int d = fromPos.depth, pos = fromPos.pos - 1; d > 0; d--, pos--) {
    ...
    if (spec.defining == true ||
        spec.definingAsContext == true ||
        spec.isolating == true) break;
  ...
  for (int j = slice.openStart; j >= 0; j--) {
    ...
    for (int i = 0; i < targetDepths.length; i++) {
      ...
      if (parent.canReplaceWith(index, index, insert.type, insert.marks)) {
        tr.replace(
            fromPos.before(targetDepth),
            expand ? toPos.after(targetDepth) : to,
            Slice(_closeFragment(slice.content, 0, slice.openStart, openDepth),
...
Fragment _closeFragment(Fragment fragment, int depth, int oldOpen, int newOpen,
    [PMNode? parent]) {
  if (depth < oldOpen) {
    PMNode first = fragment.firstChild!;
  ...
  if (depth > newOpen) {
    ...
    fragment = start
        .append(match.matchFragment(start)!.fillBefore(Fragment.empty, true)!);
  }
  return fragment;
}
...
void deleteRange(Transform tr, int from, int to) {
  ...
  if (fromPos.parent.isTextblock &&
      ...
      fromPos.start() != toPos.start() &&
      fromPos.parentOffset == 0 &&
      toPos.parentOffset == 0) {
    ...
    if (!isolated) {
      for (int d = fromPos.depth; d > 0 && from == fromPos.start(d); d--)
        from = fromPos.before(d);
      for (int d = toPos.depth; d > 0 && to == toPos.start(d); d--)
        to = toPos.before(d);
  ...
  for (int i = 0; i < covered.length; i++) {
    ...
    if ((last && depth == 0) ||
        fromPos.node(depth).type.contentMatch.validEnd) {
      tr.delete(fromPos.start(depth), toPos.end(depth));
      return;
    }
    if (depth > 0 &&
        (last ||
            fromPos.node(depth - 1).canReplace(
                fromPos.index(depth - 1), toPos.indexAfter(depth - 1)))) {
      tr.delete(fromPos.before(depth), toPos.after(depth));
      return;
    }
  }
  for (int d = 1; d <= fromPos.depth && d <= toPos.depth; d++) {
    if (from - fromPos.start(d) == fromPos.depth - d &&
        to > fromPos.end(d) &&
        toPos.end(d) - to != toPos.depth - d &&
        fromPos.start(d - 1) == toPos.start(d - 1) &&
        fromPos
            .node(d - 1)
            .canReplace(fromPos.index(d - 1), toPos.index(d - 1))) {
...
List<int> _coveredDepths(ResolvedPos fromPos, ResolvedPos toPos) {
  ...
  for (int d = minDepth; d >= 0; d--) {
    ...
    if (start < fromPos.pos - (fromPos.depth - d) ||
        toPos.end(d) > toPos.pos + (toPos.depth - d) ||
        fromPos.node(d).type.spec.isolating == true ||
        toPos.node(d).type.spec.isolating == true) {
      break;
    }
    if (start == toPos.start(d) ||
        (d == fromPos.depth &&
            d == toPos.depth &&
            fromPos.parent.inlineContent &&
            toPos.parent.inlineContent &&
            d > 0 &&
            toPos.start(d - 1) == start - 1)) {
      result.add(d);
    }
  }
  return result;
