...
int? liftTarget(NodeRange range) {
  ...
  Fragment content =
      parent.content.cutByIndex(range.startIndex, range.endIndex);
  for (int depth = range.depth, contentBefore = 0, contentAfter = 0;; --depth) {
    ...
    if (depth == 0 ||
        ...
        !_canCut(node, index, endIndex)) break;
...
List<Wrapping>? findWrapping(NodeRange range, NodeType nodeType,
    [Map<String, dynamic>? attrs, NodeRange? innerRange]) {
  innerRange ??= range;
  List<NodeType>? around = _findWrappingOutside(range, nodeType);
  List<NodeType>? inner =
      around != null ? _findWrappingInside(innerRange, nodeType) : null;
...
void wrap(Transform tr, NodeRange range, List<Wrapping> wrappers) {
  ...
  for (int i = wrappers.length - 1; i >= 0; i--) {
    if (content.size > 0) {
      ContentMatch? match =
          wrappers[i].type.contentMatch.matchFragment(content);
  ...
  int end = range.end;
  tr.step(ReplaceAroundStep(
      start, end, start, end, Slice(content, 0, 0), wrappers.length, true));
}

void setBlockType(Transform tr, int from, int to, NodeType type,
    [dynamic attrs]) {
  if (!type.isTextblock)
  ...
  tr.doc.nodesBetween(from, to, (node, pos, parent, index) {
    ...
    if (node.isTextblock &&
      ...
      clearIncompatible(tr, tr.mapping.slice(mapFrom).map(pos, 1), type, null,
  ...
  node.forEach((child, offset, index) {
    if (child.isText) {
      ...
      for (Match m in matches) {
        ...
        tr.replaceWith(
            start, start + 1, node.type.schema.linebreakReplacement!.create());
      }
    }
  });
}
...
void setNodeMarkup(Transform tr, int pos,
  ...
  tr.step(ReplaceAroundStep(pos, pos + node.nodeSize, pos + 1,
      pos + node.nodeSize - 1, Slice(Fragment.from(newNode), 0, 0), 1, true));
}

bool canSplit(PMNode doc, int pos,
    [int depth = 1, List<Wrapping?>? typesAfter]) {
  ResolvedPos posRes = doc.resolve(pos);
  int base = posRes.depth - depth;
  Wrapping? innerType =
      (typesAfter != null && typesAfter.isNotEmpty) ? typesAfter.last : null;
  NodeType innerNodeType =
      innerType != null ? innerType.type : posRes.parent.type;

  if (base < 0 ||
      posRes.parent.type.spec.isolating == true ||
      !posRes.parent.canReplace(posRes.index(), posRes.parent.childCount) ||
      !innerNodeType.validContent(posRes.parent.content
          .cutByIndex(posRes.index(), posRes.parent.childCount))) {
    return false;
  }
  for (int d = posRes.depth - 1, i = depth - 2; d > base; d--, i--) {
    ...
    Wrapping? overrideChild = typesAfter != null && i + 1 < typesAfter.length
        ? typesAfter[i + 1]
        : null;
    if (overrideChild != null) {
      rest =
          rest.replaceChild(0, overrideChild.type.create(overrideChild.attrs));
    }
    Wrapping? afterType = typesAfter != null && i >= 0 && i < typesAfter.length
        ? typesAfter[i]
        : null;
    NodeType after = afterType != null ? afterType.type : node.type;
    if (!node.canReplace(index + 1, node.childCount) ||
        !after.validContent(rest)) return false;
  }
  int index = posRes.indexAfter(base);
  Wrapping? baseType =
      typesAfter != null && typesAfter.isNotEmpty ? typesAfter[0] : null;
  return posRes.node(base).canReplaceWith(index, index,
      baseType != null ? baseType.type : posRes.node(base + 1).type);
}

void split(Transform tr, int pos,
    [int depth = 1, List<Wrapping?>? typesAfter]) {
  ResolvedPos posRes = tr.doc.resolve(pos);
  Fragment before = Fragment.empty;
  Fragment after = Fragment.empty;
  for (int d = posRes.depth, e = posRes.depth - depth, i = depth - 1;
      d > e;
      d--, i--) {
    before = Fragment.from(posRes.node(d).copy(before));
    Wrapping? typeAfter = typesAfter != null && i >= 0 && i < typesAfter.length
        ? typesAfter[i]
        : null;
    after = Fragment.from(typeAfter != null
        ? typeAfter.type.create(typeAfter.attrs, after)
        : posRes.node(d).copy(after));
  }
  tr.step(
...
bool canJoin(PMNode doc, int pos) {
  ResolvedPos posRes = doc.resolve(pos);
  int index = posRes.index();
  return _joinable(posRes.nodeBefore, posRes.nodeAfter) &&
      posRes.parent.canReplace(index, index + 1);
}
...
bool _canAppendWithSubstitutedLinebreaks(PMNode a, PMNode b) {
  ...
  for (int i = 0; i < b.childCount; i++) {
    ...
    NodeType type = child.type == linebreakReplacement
...
int? joinPoint(PMNode doc, int pos, [int dir = -1]) {
  ...
  for (int d = posRes.depth;; d--) {
    ...
    if (before != null &&
        ...
        posRes.node(d).canReplace(index, index + 1)) {
      return pos;
    }
    if (d == 0) break;
...
Transform join(Transform tr, int pos, int depth) {
  ...
  if (beforeType.inlineContent) {
    clearIncompatible(
        ...
        pos + depth - 1,
        beforeType,
        beforePos.node().contentMatchAt(beforePos.index()),
        convertNewlines == null);
  }
...
int? insertPoint(PMNode doc, int pos, NodeType nodeType) {
  ResolvedPos posRes = doc.resolve(pos);
  if (posRes.parent.canReplaceWith(posRes.index(), posRes.index(), nodeType))
    return pos;
  if (posRes.parentOffset == 0) {
    for (int d = posRes.depth - 1; d >= 0; d--) {
      int index = posRes.index(d);
      if (posRes.node(d).canReplaceWith(index, index, nodeType))
        return posRes.before(d + 1);
...
int? dropPoint(PMNode doc, int pos, Slice slice) {
  ...
  for (int pass = 1;
      pass <= (slice.openStart == 0 && slice.size > 0 ? 2 : 1);
      pass++) {
    for (int d = posRes.depth; d >= 0; d--) {
      int bias = d == posRes.depth
          ? 0
          : posRes.pos <= (posRes.start(d + 1) + posRes.end(d + 1)) / 2
              ? -1
              : 1;
