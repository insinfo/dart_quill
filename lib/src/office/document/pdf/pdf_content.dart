...
class PdfContentBuilder {
  ...
  void _setDash(List<double>? dashPx) {
    ...
    if (wantDash) {
      final String pattern = dashPx.map((double d) => _n(d * k)).join(' ');
  ...
  void drawImage(String resourceName, double x, double y, double w, double h) {
    if (w <= 0 || h <= 0) return;
    _ops
      ..writeln('q')
