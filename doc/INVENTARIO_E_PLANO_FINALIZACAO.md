...
**Baseline de testes:** 489 unitários VM + 14 browser/Chrome + 3 E2E (Puppeteer) verdes; `dart analyze` limpo. *(atualizado em 2026-07-28)*
...
- [x] G6.8 Módulo `Table`: `showTools`/`updateMenus`/`hideTools` (agora escondendo os menus), `insertTable` terminando em `showTools`, e as opções `menus`/`toolbarButtons`/`toolbarTable` da interface TS entregues em G6.2/G6.3. **Handlers de raiz concluídos em 2026-07-28** (quill-table-better.ts:96-98/103/147-212): o módulo passa a escutar `keyup`, `mousedown` e `scroll` na raiz do editor e a ser dono do ciclo de vida em volta da grade (T8, T9).
  ...
  - **Desvio documentado no cabeçalho do arquivo:** o upstream tem uma `CellSelection` por editor, e por isso o `handleKeyup`/`handleMousedown` dele delegam explicitamente a ela. Este port tem um `CellSelectionController` por tabela, cada um já escutando a mesma raiz e ignorando eventos fora da sua tabela — as chamadas de delegação somem, o resto fica fiel. 15 testes em `test/unit/table_better/module_handlers_test.dart`.
