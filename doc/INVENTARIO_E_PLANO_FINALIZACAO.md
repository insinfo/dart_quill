...
- [x] **G10.11 `getSemanticHTML` fiel, provado por golden em todos os 48 casos (2026-07-29).** Fecha o pendente do C3/G2.3 (`getHTML`/`convertHTML`). O gerador passou a gravar **`semantic: quill.getSemanticHTML()`** de cada caso core (regeração 49/49 reproduzível fora o campo novo), e `goldens_test` compara byte a byte. O serializer bespoke de ~350 linhas (`deltaToSemanticHTML` + 15 helpers em quill.dart) **foi removido**; entraram os fiéis `Editor.getHTML` (editor.ts:198-209), `convertHTML`/`convertListHTML`/`getListType` (editor.ts:326-449, incluindo o truque `outerHTML.split('>${innerHTML}<')` — `DomElement.outerHTML` entrou na abstração DOM e no fake, com o serializer emitindo o shortcut `_text` para manter outer/inner consistentes) e o hook **`Blot.html([index, length])`** (o `'html' in blot`), unificando as assinaturas dos seis `html()` que já existiam **sem nenhum chamador**. Colaterais fiéis: `escapeText` com o mapa exato do text.ts (o `HtmlEscape` do Dart escapa `/` a mais); `Formula.html` perdeu o wrapper `$$…$$` inventado; `CodeBlockContainer.code()` lê o texto do DOM (`value()` de Block devolvia uma List → `[line1]` no HTML) com clamp de `slice`; e **`_writeInlineStyles` serializa como o cssText do browser** (hex→`rgb(r, g, b)`, declarações terminadas em `;`) — o parchment escreve via CSSOM e o browser re-serializa assim, enquanto `setAttribute('style')` fica verbatim (é por isso que o golden tem `width: 100%` sem `;` no temporary e `rgb(255, 0, 0);` na cor). 564 VM + 14 Chrome + 3 E2E verdes.

- [x] **G10.12 Goldens `semantic` estendidos ao table-better — 19/19 com HTML serializado (2026-07-29).** O harness do plugin também grava `getSemanticHTML()` por caso, e o comparador ganhou `_canonicaliseHtml` (ids `data-cell`/`data-row` canonizados por ordem de aparição dentro da string, e atributos ordenados dentro de cada tag — browser serializa em ordem de inserção, o fake na dele; a ordem não carrega significado). Os `html()` dos blots do plugin, até então código morto, passaram a ter prova. **Quatro defeitos reais encontrados e corrigidos:**
  1. **O `<table>` do port nunca ganhava `class="ql-table-better"` em VM.** A cadeia é de duas passadas (container escreve `data-class` no temporary; o temporary sincroniza para a tabela na seguinte) e o loop de convergência off-browser só reage a `treeVersion` estrutural — a escrita de atributo não disparava a passada extra que o MutationObserver dispara no browser. `_setClassName` agora bumpa `treeVersion` quando muda o `data-class`.
  2. **Fake DOM escondia estado, de novo (dois casos):** `setAttribute('class', …)` e o `classList` eram **dois estados separados** — `getAttribute` via a classe que o `outerHTML` não serializava; unificados com `_classes` como fonte única, incluindo o detalhe de browser de que **qualquer mutação do classList materializa o atributo** (`classList.remove` num elemento sem classe deixa `class=""` — o golden do deleteTable depende disso). E a lista de void elements não tinha `col` (nem area/base/embed/source/track/wbr) → `<col></col>` no HTML serializado.
  3. **Todo `<li>` de tabela saía com dois spans `.ql-ui`**: `TableList.create` construía um `ListItem` inteiro (cujo construtor anexa o span) só para pegar o elemento, e o construtor do `TableList` anexava o segundo. O nó agora é construído direto.
  4. **`setSelected` pintava `ql-cell-selected` na âncora** — o TS (cell-selection.ts:709-721) mantém a célula na lista de seleção mas pinta só `ql-cell-focused`.
  564 VM + 14 Chrome + 3 E2E verdes.
