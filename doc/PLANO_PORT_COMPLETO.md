...
- [ ] F5.6 `ui/cell-selection.ts` (811 l.) → **parcial:** núcleo `CellSelection` com retângulo lógico normalizado, expansão de `rowspan`/`colspan` e classe `ql-cell-selected` entregue; ainda faltam hit-test/arraste visual, copy/cut/paste multicélula, navegação por setas e `WHITE_LIST`.
...
**Incremento seguinte:** núcleo de seleção multicélula coberto; estado atual: 209 unitários + 3 E2E.
