...
- [ ] F5.4 `quill-table-better.ts` (446 l.) → **parcial:** módulo `TableBetter`, registro `modules/table-better`, `insertTable`/`deleteTable`/`getTable`, limpeza de temporários, `listenDeleteTable`, prevenção de tabela aninhada e wiring de `CellSelectionController` entregues; ainda faltam os keyboard bindings `makeTableArrowHandler`/`makeCellBlockHandler`/etc. e a composição final das UIs.
...
**Incremento seguinte:** núcleo do módulo `TableBetter` e caminho delta→DOM cobertos; estado atual: 230 unitários + 3 E2E + 7 browser no Chrome.
