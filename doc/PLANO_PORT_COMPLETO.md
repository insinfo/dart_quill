...
**Estado em 2026-07-11:** o módulo básico `modules/table.dart` está funcional e habilitado por padrão. A toolbar Snow e os demos usam um único botão com dropdown em grade 10×10, realce progressivo e rótulo linhas×colunas, portado de `ui/toolbar-table.ts`. Ao entrar numa célula surge uma mini-toolbar contextual com inserir linha acima/abaixo, inserir coluna à esquerda/direita, excluir linha/coluna/tabela e merge/split horizontal, usando títulos e ícones Tabler locais. Do table-better já existem formatos estruturais, config, utils, idiomas en_US/pt_BR e clipboard avançado opt-in. Seleção multicélula, merge retangular/rowspan, propriedades e redimensionamento visual ainda não estão concluídos.
...
- [ ] F5.3 `formats/header.ts` (78 l.) e `formats/list.ts` (159 l.) → **parcial:** `TableHeader`, `TableListContainer` e `TableList`, hidratação por classe, preservação de `data-cell`, conversão para `TableCellBlock` e bindings de edição entregues; ainda faltam todas as conversões cruzadas header↔list e roteamento avançado da toolbar.
- [ ] F5.4 `quill-table-better.ts` (446 l.) → **parcial:** módulo `TableBetter`, registro `modules/table-better`, `insertTable`/`deleteTable`/`getTable`, limpeza de temporários, `listenDeleteTable`, prevenção de tabela aninhada, wiring de `CellSelectionController` e bindings de célula/header/list entregues; ainda falta a composição final das UIs.
...
- [ ] F5.8 `ui/table-menus.ts` (1059 l.) → **parcial:** mini-toolbar contextual entregue para inserir/apagar linha/coluna/tabela e merge/split horizontal da célula ativa com `colspan`; **merge retangular/rowspan, split multicélula, header row (`convertToHeaderRow`/`convertToRow`) e payload de copiar tabela entregues em `CellSelection`** (2026-07-11); ainda faltam os menus dropdown DOM completos e o roteamento pela mini-toolbar.
...
**Incremento seguinte:** formatos header/list, bindings e roteamento de formatos de bloco da toolbar cobertos; estado atual: 234 unitários + 3 E2E + 7 browser no Chrome.

**Incremento de 2026-07-11 (requiredContainer/delta→DOM):** portado o mecanismo `requiredContainer` do parchment (table.ts:851-872) para os blots table-better: `TableBetterContainer` ganhou `requiredContainerBlotName`/`isRequiredContainer` com convergência local (join no container à esquerda + re-otimização do adotante, já que `Scroll.optimize` do Dart é passada única, sem o fixpoint por mutações do parchment); enforcement também em `TableCellBlock` (com guard pela classe `ql-table-block`), `TableCol` e `TableTemporary`. `Editor.update` agora usa `scroll.batchStart()`/`batchEnd()` (paridade editor.ts:31/120) — sem isso o optimize rodava entre os `formatAt` de uma mesma linha e embrulhava prematuramente. Resultado: o delta de `insertTable` (quill-table-better.ts:224) hidrata numa `<table>` única bem formada (temporary embutido, tbody, linhas mescladas por `data-row`) — pré-requisito direto de F5.4 (módulo TableBetter), do paste de tabelas e do import DOCX de tabelas. Testes novos: `delta_apply_test.dart` (2) + auto-wrap em `formats_test.dart`. Estado: 227 unit + 3 E2E + 7 browser verdes.

**Incremento de 2026-07-11 (merge retangular):** `CellSelection.mergeCells`/`splitCells` portados de `table-menus.ts:834/924` sobre a grade lógica (sem aritmética de pixels): merge retangular com `colspan`+`rowspan`, remoção de linhas esvaziadas com encolhimento dos `rowspan` irmãos, split expandindo célula mesclada em 1×1 por linha/coluna com referência lógica (`getRefInfo` lógico). Também portados `convertToHeaderRow`/`convertToRow` (header row `thead`/`th` bidirecional, table-menus.ts:297/329) e `copyTableData` (payload de `copyTable`, table-menus.ts:359). Estado: 224 unitários + 3 E2E verdes. Pendências F5.8 restantes: menus dropdown completos (DOM), integração do merge com a mini-toolbar do módulo básico e `insertColumn` com células mescladas atravessadas.
