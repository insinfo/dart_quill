...
- [ ] F5.6 `ui/cell-selection.ts` (811 l.) → **parcial:** núcleo `CellSelection` com retângulo lógico normalizado, expansão de `rowspan`/`colspan`, classe `ql-cell-selected` e controlador opt-in de âncora/`Shift`-click entregues; ainda faltam hit-test/arraste visual, copy/cut/paste multicélula, navegação por setas e `WHITE_LIST`.
...
**Incremento seguinte:** seleção lógica e `Shift`-click multicélula cobertos; estado atual: 210 unitários + 3 E2E.
