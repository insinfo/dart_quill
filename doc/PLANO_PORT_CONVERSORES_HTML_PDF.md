...
- [x] **P1.1 Parser TTF/OTF (2026-07-31)** — `lib/src/office/document/fonts/truetype.dart`. Lê `head`, `hhea`, `hmtx`, `maxp`, `cmap` (formatos 0, 4, 6 e 12), `loca`, `glyf`, `name`, `post` e `OS/2`. Expõe o que embutir exige: `glyphIdFor`, `advanceWidthOf`/`advanceWidthOfChar` (em milésimos de em, a unidade do PDF), `glyphData`, `componentGlyphsOf`/`expandWithComponents`, métricas, estilo e `embeddingPermission` (`OS/2.fsType` — a fonte pode proibir ser embutida, e gerar documento que viola isso é problema de quem gera). **25 casos** contra a **Inter-Regular real**, não contra um arquivo sintético: um sintético só provaria que o parser lê o que eu mesmo escrevi.
  - *Três armadilhas do formato que os testes cobrem:* a `hmtx` tem **menos** entradas que glifos quando a cauda é monoespaçada (os últimos repetem a última largura — ignorar isso mede errado o fim do alfabeto); o `idRangeOffset` do `cmap` formato 4 é relativo **à própria posição dele**, a indireção mais fácil de errar do formato; e um acentuado costuma ser **glifo composto**, então um subconjunto que esqueça os componentes desenha buracos exatamente nos caracteres acentuados.
  - *Premissa minha que o teste derrubou:* eu assumi que a área de uso privado (U+E000) não teria glifo. A Inter mapeia — U+E000 → glifo 1863. O caso de "caractere ausente" passou a usar CJK, tailandês e emoji, medidos.
  - *Fixture:* `test/assets/fonts/Inter-Regular.ttf` (SIL OFL, redistribuível) com README de procedência. Fica em `test/`, **nunca** em `lib/` — a decisão 1 continua valendo.
- [x] **P1.2 Subsetting (2026-07-31)** — `lib/src/office/document/fonts/truetype_subset.dart`. Reescreve `glyf`/`loca`/`hmtx`/`maxp`/`hhea`/`head`, copia as tabelas de hinting (`cvt `, `fpgm`, `prep`) e remonta o sfnt com os *checksums* corretos, inclusive o `checkSumAdjustment` — a assinatura que valida a fonte inteira e que, errada, faz validadores recusarem o arquivo.
  - **Medido na Inter (411 KB):** um despacho realista com acentuação usa 63 glifos e sai em **33 KB (8,0%)**; "Despacho de teste", 12 glifos, em **21 KB (5,1%)**. Para as quatro variantes que o SALI embute, é ~130 KB no lugar de ~1,6 MB.
  - **Decisão: os identificadores de glifo são preservados** (`numGlyphs = maiorGlifoMantido + 1`), o que permite `Identity-H` com CID = GID e dispensa um `CIDToGIDMap` no PDF. O custo aparece na medição: como as maiúsculas acentuadas ficam no fim da Inter, a `loca` precisa de 2.715 entradas e responde por cerca de um terço do subset. Renumerar encolheria mais, ao preço de um mapa extra no PDF e de uma classe inteira de erros de correspondência — não paga.
  - **15 casos.** O critério que vale não é "o arquivo encolheu": é que a fonte recortada seja **relida** e devolva, para cada caractere mantido, o mesmo contorno, o mesmo avanço e o mesmo lsb da original. Um subsetter que gere arquivo menor e desenhe errado é pior que nenhum, porque o defeito só aparece no PDF do usuário. Cobertos também: componentes de acentuados (`ção`), `.notdef` sempre presente, espaço sem contorno mas com avanço, glifo descartado com comprimento zero, diretório apontando para dentro do arquivo, e **determinismo** — bytes instáveis quebrariam o hash SHA-256 do documento assinado.
  - *Correção que o subset exigiu no parser:* `leftSideBearingOf`. A `hmtx` guarda `numberOfHMetrics` pares (avanço, lsb) e **depois deles só lsb**; ler isso errado desloca o desenho dos últimos glifos dentro da própria caixa.
- [x] **P1.3 Embedding CID (2026-07-31)** — `lib/src/office/document/pdf/pdf_cid_font.dart`. Monta a cadeia do PDF 32000-1 §9.7: `/Type0` + `/Identity-H` → `/CIDFontType2` (com `/W`, `/DW 1000` e `/CIDToGIDMap /Identity`) → `/FontDescriptor` (flags, bbox, ascent/descent/capHeight/StemV) → `/FontFile2` com o subset comprimido e `/Length1`. O `PdfWriter` ganhou `registerFontResource`, para uma fonte embutida entrar nos `/Resources` da página sem se misturar às standard-14.
  - **`/ToUnicode` desde o primeiro dia.** Ele não muda um pixel do desenho e decide se copiar texto do documento assinado devolve as palavras ou lixo — sem ele o PDF é visualmente perfeito e inútil para busca, leitor de tela e conferência. Emitido como CMap com blocos de no máximo 100 entradas (limite do formato) e pares substitutos para fora do BMP.
  - **O prefixo de subconjunto é derivado dos glifos (FNV-1a), não aleatório.** Um prefixo sorteado mudaria os bytes a cada execução e quebraria o hash SHA-256 do documento assinado — que é a razão de o determinismo estar fixado em teste desde o P0.
  - **Medido, ponta a ponta:** um despacho realista com acentuação vira um **PDF de 13 KB** com a Inter embutida (subset de 32 KB antes da compressão do stream, sobre uma fonte de 411 KB).
  - **17 casos**, em duas frentes independentes: a **estrutura** (um elo solto na cadeia faz o leitor cair numa fonte substituta, em silêncio) e o **`/ToUnicode`** (cada caractere do texto tem de aparecer com o seu ponto de código). Mais: caractere sem glifo vira `.notdef` visível em vez de sumir, a medição usa a métrica real, e uma fonte que declara em `OS/2.fsType` que não pode ser embutida é **recusada com mensagem clara**, não embutida assim mesmo.
