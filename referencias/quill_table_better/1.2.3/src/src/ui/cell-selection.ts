...
class CellSelection {
  ...
  onCapturePaste(e: ClipboardEvent) {
    ...
    // SALI: listener global de paste no document — startTd pode estar órfão
    // (tabela substituída desde a última seleção) e Quill.find retorna null;
    // sem o guard: "Cannot read properties of null (reading 'row')".
    if (!cell) return;
    const row = cell.row();
    const table = cell.table();
