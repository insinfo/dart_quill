import 'package:dart_quill/src/blots/abstract/blot.dart';
import 'package:dart_quill/src/blots/block.dart';
import 'package:dart_quill/src/blots/scroll.dart';
import 'package:dart_quill/src/core/editor.dart';
import 'package:dart_quill/src/core/emitter.dart';
import 'package:dart_quill/src/core/selection.dart';
import 'package:dart_quill/src/dependencies/dart_quill_delta/dart_quill_delta.dart';
import 'package:dart_quill/src/platform/dom.dart';
import 'package:test/test.dart';

import '../../support/quill_test_helpers.dart';
import '../../support/test_helpers.dart';

/// Port of `referencias/quilljs/test/unit/core/editor.spec.ts`.
///
/// Every case keeps the upstream name, order and assertions: the delta is read
/// back with `Editor.getDelta()` (recomputed from the blot tree, not the cached
/// snapshot) and the DOM is compared with the upstream `toEqualHTML` helper.

/// The block embed the spec uses throughout (`editor.spec.ts:815-816`).
const String _video = '<iframe src="#" class="ql-video" frameborder="0" '
    'allowfullscreen="true"></iframe>';

class _MyBlot extends Block {
  _MyBlot(super.domNode);

  @override
  String get blotName => 'my-blot';

  @override
  Map<String, dynamic> formats() => {
        ...super.formats(),
        'my-blot': true,
      };

  @override
  _MyBlot clone() => _MyBlot(element.cloneNode(deep: false));

  @override
  void formatAt(int index, int length, String name, dynamic value) {
    super.formatAt(index, length, name, value);
    if (name == 'test-style' && prev != null) {
      element.setAttribute('test-style', '$value');
    }
  }
}

void main() {
  group('Editor', () {
    group('insert', () {
      test('text', () {
        final editor = createEditor('<p><strong>0123</strong></p>');
        editor.insertText(2, '!!');
        expectDelta(
          editor.getDelta(),
          Delta()
            ..insert('01!!23', {'bold': true})
            ..insert('\n'),
        );
        expect(
            editor.scroll.element, EqualHTML('<p><strong>01!!23</strong></p>'));
      });

      test('embed', () {
        final editor = createEditor('<p><strong>0123</strong></p>');
        editor.insertEmbed(2, 'image', '/assets/favicon.png');
        expectDelta(
          editor.getDelta(),
          Delta()
            ..insert('01', {'bold': true})
            ..insert({'image': '/assets/favicon.png'}, {'bold': true})
            ..insert('23', {'bold': true})
            ..insert('\n'),
        );
        expect(
          editor.scroll.element,
          EqualHTML(
              '<p><strong>01<img src="/assets/favicon.png">23</strong></p>'),
        );
      });

      test('on empty line', () {
        final editor = createEditor('<p>0</p><p><br></p><p>3</p>');
        editor.insertText(2, '!');
        expectDelta(editor.getDelta(), Delta()..insert('0\n!\n3\n'));
        expect(editor.scroll.element, EqualHTML('<p>0</p><p>!</p><p>3</p>'));
      });

      test('end of document', () {
        final editor = createEditor('<p>Hello</p>');
        editor.insertText(6, 'World!');
        expectDelta(editor.getDelta(), Delta()..insert('Hello\nWorld!\n'));
        expect(editor.scroll.element, EqualHTML('<p>Hello</p><p>World!</p>'));
      });

      test('end of document with newline', () {
        final editor = createEditor('<p>Hello</p>');
        editor.insertText(6, 'World!\n');
        expectDelta(editor.getDelta(), Delta()..insert('Hello\nWorld!\n'));
        expect(editor.scroll.element, EqualHTML('<p>Hello</p><p>World!</p>'));
      });

      test('embed at end of document with newline', () {
        final editor = createEditor('<p>Hello</p>');
        editor.insertEmbed(6, 'image', '/assets/favicon.png');
        expectDelta(
          editor.getDelta(),
          Delta()
            ..insert('Hello\n')
            ..insert({'image': '/assets/favicon.png'})
            ..insert('\n'),
        );
        expect(
          editor.scroll.element,
          EqualHTML('<p>Hello</p><p><img src="/assets/favicon.png"></p>'),
        );
      });

      test('newline splitting', () {
        final editor = createEditor('<p><strong>0123</strong></p>');
        editor.insertText(2, '\n');
        expectDelta(
          editor.getDelta(),
          Delta()
            ..insert('01', {'bold': true})
            ..insert('\n')
            ..insert('23', {'bold': true})
            ..insert('\n'),
        );
        expect(
          editor.scroll.element,
          EqualHTML('''
            <p><strong>01</strong></p>
            <p><strong>23</strong></p>'''),
        );
      });

      test('prepend newline', () {
        final editor = createEditor('<p><strong>0123</strong></p>');
        editor.insertText(0, '\n');
        expectDelta(
          editor.getDelta(),
          Delta()
            ..insert('\n')
            ..insert('0123', {'bold': true})
            ..insert('\n'),
        );
        expect(
          editor.scroll.element,
          EqualHTML('''
            <p><br></p>
            <p><strong>0123</strong></p>'''),
        );
      });

      test('append newline', () {
        final editor = createEditor('<p><strong>0123</strong></p>');
        editor.insertText(4, '\n');
        expectDelta(
          editor.getDelta(),
          Delta()
            ..insert('0123', {'bold': true})
            ..insert('\n\n'),
        );
        expect(
          editor.scroll.element,
          EqualHTML('''
            <p><strong>0123</strong></p>
            <p><br></p>'''),
        );
      });

      test('multiline text', () {
        final editor = createEditor('<p><strong>0123</strong></p>');
        editor.insertText(2, '\n!!\n!!\n');
        expectDelta(
          editor.getDelta(),
          Delta()
            ..insert('01', {'bold': true})
            ..insert('\n')
            ..insert('!!', {'bold': true})
            ..insert('\n')
            ..insert('!!', {'bold': true})
            ..insert('\n')
            ..insert('23', {'bold': true})
            ..insert('\n'),
        );
        expect(
          editor.scroll.element,
          EqualHTML('''
            <p><strong>01</strong></p>
            <p><strong>!!</strong></p>
            <p><strong>!!</strong></p>
            <p><strong>23</strong></p>'''),
        );
      });

      test('multiple newlines', () {
        final editor = createEditor('<p><strong>0123</strong></p>');
        editor.insertText(2, '\n\n');
        expectDelta(
          editor.getDelta(),
          Delta()
            ..insert('01', {'bold': true})
            ..insert('\n\n')
            ..insert('23', {'bold': true})
            ..insert('\n'),
        );
        expect(
          editor.scroll.element,
          EqualHTML('''
            <p><strong>01</strong></p>
            <p><br></p>
            <p><strong>23</strong></p>'''),
        );
      });

      test('text removing formatting', () {
        final editor = createEditor('<p><s>01</s></p>');
        editor.insertText(2, '23', {'bold': false, 'strike': false});
        expectDelta(
          editor.getDelta(),
          Delta()
            ..insert('01', {'strike': true})
            ..insert('23\n'),
        );
      });
    });

    group('delete', () {
      test('inner node', () {
        final editor = createEditor('<p><strong><em>0123</em></strong></p>');
        editor.deleteText(1, 2);
        expectDelta(
          editor.getDelta(),
          Delta()
            ..insert('03', {'bold': true, 'italic': true})
            ..insert('\n'),
        );
        expect(editor.scroll.element,
            EqualHTML('<p><strong><em>03</em></strong></p>'));
      });

      test('parts of multiple lines', () {
        final editor = createEditor('<p><em>0123</em></p><p><em>5678</em></p>');
        editor.deleteText(2, 5);
        expectDelta(
          editor.getDelta(),
          Delta()
            ..insert('0178', {'italic': true})
            ..insert('\n'),
        );
        expect(editor.scroll.element, EqualHTML('<p><em>0178</em></p>'));
      });

      test('entire line keeping newline', () {
        final editor = createEditor('<p><strong><em>0123</em></strong></p>');
        editor.deleteText(0, 4);
        expectDelta(editor.getDelta(), Delta()..insert('\n'));
        expect(editor.scroll.element, EqualHTML('<p><br></p>'));
      });

      test('newline', () {
        final editor = createEditor('<p><em>0123</em></p><p><em>5678</em></p>');
        editor.deleteText(4, 1);
        expectDelta(
          editor.getDelta(),
          Delta()
            ..insert('01235678', {'italic': true})
            ..insert('\n'),
        );
        expect(editor.scroll.element, EqualHTML('<p><em>01235678</em></p>'));
      });

      test('entire document', () {
        final editor = createEditor('<p><strong><em>0123</em></strong></p>');
        editor.deleteText(0, 5);
        expectDelta(editor.getDelta(), Delta()..insert('\n'));
        expect(editor.scroll.element, EqualHTML('<p><br></p>'));
      });

      test('multiple complete lines', () {
        final editor = createEditor(
          '<p><em>012</em></p><p><em>456</em></p><p><em>890</em></p>',
        );
        editor.deleteText(0, 8);
        expectDelta(
          editor.getDelta(),
          Delta()
            ..insert('890', {'italic': true})
            ..insert('\n'),
        );
        expect(editor.scroll.element, EqualHTML('<p><em>890</em></p>'));
      });
    });

    group('format', () {
      test('line', () {
        final editor = createEditor('<p>0123</p>');
        editor.formatLine(1, 1, {'header': 1});
        expect(editor.scroll.element, EqualHTML('<h1>0123</h1>'));
      });
    });

    group('removeFormat', () {
      test('unwrap', () {
        final editor = createEditor('<p>0<em>12</em>3</p>');
        editor.removeFormat(1, 2);
        expect(editor.scroll.element, EqualHTML('<p>0123</p>'));
      });

      test('split inline', () {
        final editor = createEditor('<p>0<strong><em>12</em></strong>3</p>');
        editor.removeFormat(1, 1);
        expect(editor.scroll.element,
            EqualHTML('<p>01<strong><em>2</em></strong>3</p>'));
      });

      test('partial line', () {
        final editor = createEditor(
          '<h1>01</h1><ol><li data-list="ordered">34</li></ol>',
        );
        editor.removeFormat(1, 3);
        expect(editor.scroll.element, EqualHTML('<p>01</p><p>34</p>'));
      });

      test('remove embed', () {
        final editor = createEditor('<p>0<img src="/assets/favicon.png">2</p>');
        editor.removeFormat(1, 1);
        expect(editor.scroll.element, EqualHTML('<p>02</p>'));
      });

      test('combined', () {
        final editor = createEditor('''
          <h1>01<img src="/assets/favicon.png">3</h1>
          <ol>
            <li data-list="ordered">5<strong>6<em>78</em>9</strong>0</li>
          </ol>
        ''');
        editor.removeFormat(1, 7);
        expect(
          editor.scroll.element,
          EqualHTML('''
            <p>013</p>
            <p>567<strong><em>8</em>9</strong>0</p>
          '''),
        );
      });

      test('end of document', () {
        final editor = createEditor('''
          <ol>
            <li data-list="ordered">0123</li>
            <li data-list="ordered">5678</li>
          </ol>
        ''');
        editor.removeFormat(0, 12);
        expect(
          editor.scroll.element,
          EqualHTML('''
            <p>0123</p>
            <p>5678</p>
          '''),
        );
      });
    });

    group('applyDelta', () {
      test('insert', () {
        final editor = createEditor('<p></p>');
        editor.applyDelta(Delta()..insert('01'));
        expect(editor.scroll.element, EqualHTML('<p>01</p>'));
      });

      test('attributed insert', () {
        final editor = createEditor('<p>0123</p>');
        editor.applyDelta(Delta()
          ..retain(2)
          ..insert('|', {'bold': true}));
        expect(
            editor.scroll.element, EqualHTML('<p>01<strong>|</strong>23</p>'));
      });

      test('format', () {
        final editor = createEditor('<p>01</p>');
        editor.applyDelta(Delta()..retain(2, {'bold': true}));
        expect(editor.scroll.element, EqualHTML('<p><strong>01</strong></p>'));
      });

      test('discontinuous formats', () {
        final editor = createEditor('');
        final delta = Delta()
          ..insert('ab', {'bold': true})
          ..insert('23\n45')
          ..insert('cd', {'bold': true});
        editor.applyDelta(delta);
        expect(
          editor.scroll.element,
          EqualHTML('<p><strong>ab</strong>23</p><p>45<strong>cd</strong></p>'),
        );
      });

      test('unformatted insert', () {
        final editor = createEditor('<p><em>01</em></p>');
        editor.applyDelta(Delta()
          ..retain(1)
          ..insert('|'));
        expect(
            editor.scroll.element, EqualHTML('<p><em>0</em>|<em>1</em></p>'));
      });

      test('insert at format boundary', () {
        final editor = createEditor('<p><em>0</em><u>1</u></p>');
        editor.applyDelta(Delta()
          ..retain(1)
          ..insert('|', {'strike': true}));
        expect(editor.scroll.element,
            EqualHTML('<p><em>0</em><s>|</s><u>1</u></p>'));
      });

      test('unformatted newline', () {
        final editor = createEditor('<h1>01</h1>');
        editor.applyDelta(Delta()
          ..retain(2)
          ..insert('\n'));
        expect(editor.scroll.element, EqualHTML('<p>01</p><h1><br></h1>'));
      });

      test('formatted embed', () {
        final editor = createEditor('');
        editor.applyDelta(Delta()
          ..insert({'image': '/assets/favicon.png'}, {'italic': true}));
        expect(
          editor.scroll.element,
          EqualHTML('<p><em><img src="/assets/favicon.png"></em></p>'),
        );
      });

      test('insert text before block embed', () {
        final editor = createEditor('<p>0123</p>$_video');
        editor.applyDelta(Delta()
          ..retain(5)
          ..insert('5678'));
        expect(
            editor.scroll.element, EqualHTML('<p>0123</p><p>5678</p>$_video'));
      });

      test('insert attributed text before block embed', () {
        final editor = createEditor('<p>0123</p>$_video');
        editor.applyDelta(Delta()
          ..retain(5)
          ..insert('5678', {'bold': true}));
        expect(
          editor.scroll.element,
          EqualHTML('<p>0123</p><p><strong>5678</strong></p>$_video'),
        );
      });

      test('insert text with newline before block embed', () {
        final editor = createEditor('<p>0123</p>$_video');
        editor.applyDelta(Delta()
          ..retain(5)
          ..insert('5678\n'));
        expect(
            editor.scroll.element, EqualHTML('<p>0123</p><p>5678</p>$_video'));
      });

      test('insert formatted lines before block embed', () {
        final editor = createEditor('<p>0123</p>$_video');
        editor.applyDelta(Delta()
          ..retain(5)
          ..insert('a\nb')
          ..insert('\n', {'header': 1}));
        expect(
          editor.scroll.element,
          EqualHTML('<p>0123</p><p>a</p><h1>b</h1>$_video'),
        );
      });

      test('insert attributed text with newline before block embed', () {
        final editor = createEditor('<p>0123</p>$_video');
        editor.applyDelta(Delta()
          ..retain(5)
          ..insert('5678', {'bold': true})
          ..insert('\n'));
        expect(
          editor.scroll.element,
          EqualHTML('<p>0123</p><p><strong>5678</strong></p>$_video'),
        );
      });

      test('multiple inserts and deletes', () {
        final editor = createEditor('<p>0123</p>');
        editor.applyDelta(Delta()
          ..retain(1)
          ..insert('a')
          ..delete(2)
          ..insert('cd')
          ..delete(1)
          ..insert('efg'));
        expect(editor.scroll.element, EqualHTML('<p>0acdefg</p>'));
      });

      test('insert text with delete in existing block', () {
        final editor = createEditor('<p>0123</p>$_video');
        editor.applyDelta(Delta()
          ..retain(4)
          ..insert('abc')
          // Retain newline at end of block being inserted into.
          ..retain(1)
          ..delete(1));
        expect(editor.scroll.element, EqualHTML('<p>0123abc</p>'));
      });

      test('insert text with delete before block embed', () {
        final editor = createEditor('<p>0123</p>$_video');
        editor.applyDelta(Delta()
          ..retain(5)
          // Explicit newline required to maintain correct index calculation
          // for the delete.
          ..insert('abc\n')
          ..delete(1));
        expect(editor.scroll.element, EqualHTML('<p>0123</p><p>abc</p>'));
      });

      test('insert inline embed with delete in existing block', () {
        final editor = createEditor('<p>0123</p>$_video');
        editor.applyDelta(Delta()
          ..retain(4)
          ..insert({'image': '/assets/favicon.png'})
          ..retain(1)
          ..delete(1));
        expect(
          editor.scroll.element,
          EqualHTML('<p>0123<img src="/assets/favicon.png"></p>'),
        );
      });

      test('insert inline embed with delete before block embed', () {
        final editor = createEditor('<p>0123</p>$_video');
        editor.applyDelta(Delta()
          ..retain(5)
          ..insert({'image': '/assets/favicon.png'})
          ..insert('\n')
          ..delete(1));
        expect(
          editor.scroll.element,
          EqualHTML('<p>0123</p><p><img src="/assets/favicon.png"></p>'),
        );
      });

      test(
          'insert inline embed with delete before block embed using delete op first',
          () {
        final editor = createEditor('<p>0123</p>$_video');
        editor.applyDelta(Delta()
          ..retain(5)
          ..delete(1)
          ..insert({'image': '/assets/favicon.png'})
          ..insert('\n'));
        expect(
          editor.scroll.element,
          EqualHTML('<p>0123</p><p><img src="/assets/favicon.png"></p>'),
        );
      });

      test('insert inline embed and text with delete before block embed', () {
        final editor = createEditor('<p>0123</p>$_video');
        editor.applyDelta(Delta()
          ..retain(5)
          ..insert({'image': '/assets/favicon.png'})
          ..insert('abc\n')
          ..delete(1));
        expect(
          editor.scroll.element,
          EqualHTML('<p>0123</p><p><img src="/assets/favicon.png">abc</p>'),
        );
      });

      test('insert inline embed to the middle of formatted content', () {
        final editor = createEditor('<p><strong>0123</strong></p>');
        editor.applyDelta(Delta()
          ..retain(2)
          ..insert({'image': '/assets/favicon.png'}));
        expect(
          editor.scroll.element,
          EqualHTML(
            '<p><strong>01</strong><img src="/assets/favicon.png">'
            '<strong>23</strong></p>',
          ),
        );
      });

      test('insert inline embed between plain text and formatted content', () {
        final editor = createEditor('<p>a<strong>b</strong></p>');
        editor.applyDelta(Delta()
          ..retain(1)
          ..insert({'image': '#'}));
        expect(editor.scroll.element,
            EqualHTML('<p>a<img src="#"><strong>b</strong></p>'));
      });

      test('prepend inline embed to another inline embed with same attributes',
          () {
        final editor = createEditor('<p><img src="#" alt="hi"/></p>');
        editor.applyDelta(Delta()..insert({'image': '#'}, {'alt': 'hi'}));
        expect(
          editor.scroll.element,
          EqualHTML('<p><img src="#" alt="hi"><img src="#" alt="hi"></p>'),
        );
      });

      test('insert block embed with delete before block embed', () {
        final editor = createEditor('<p>0123</p>$_video');
        editor.applyDelta(Delta()
          ..retain(5)
          ..insert({'video': '#changed'})
          ..delete(1));
        expect(
          editor.scroll.element,
          EqualHTML(
            '<p>0123</p><iframe src="#changed" class="ql-video" '
            'frameborder="0" allowfullscreen="true"></iframe>',
          ),
        );
      });

      test('deletes block embed and appends text', () {
        final editor = createEditor('<p><br></p>$_video<p>b</p>');
        editor.applyDelta(Delta()
          ..retain(1)
          ..insert('a')
          ..delete(1));
        expect(editor.scroll.element, EqualHTML('<p><br></p><p>ab</p>'));
      });

      test('multiple delete block embed and append texts', () {
        final editor = createEditor('<p><br></p>$_video$_video<p>b</p>');
        editor.applyDelta(Delta()
          ..retain(1)
          ..insert('a')
          ..delete(1)
          ..insert('!')
          ..delete(1));
        expect(editor.scroll.element, EqualHTML('<p><br></p><p>a!b</p>'));
      });

      test('multiple nonconsecutive delete block embed and append texts', () {
        final editor = createEditor('''
          <p><br></p>
          $_video
          <p>a</p>
          $_video
          <p>bb</p>
          $_video
          <p>ccc</p>
          $_video
          <p>dddd</p>''');
        final old = editor.getDelta();
        final delta = Delta()
          ..retain(1)
          ..insert('1')
          ..delete(1)
          ..retain(2)
          ..insert('2')
          ..delete(1)
          ..retain(3)
          ..insert('3')
          ..delete(1)
          ..retain(4)
          ..insert('4')
          ..delete(1);
        editor.applyDelta(delta);
        expectDelta(editor.getDelta(), old.compose(delta));
        expect(
          editor.scroll.element,
          EqualHTML('<p><br></p><p>1a</p><p>2bb</p><p>3ccc</p><p>4dddd</p>'),
        );
      });

      group('block embed', () {
        test('improper block embed insert', () {
          final editor = createEditor('<p>0123</p>');
          editor.applyDelta(Delta()
            ..retain(2)
            ..insert({'video': '#'}));
          expect(editor.scroll.element, EqualHTML('<p>01</p>$_video<p>23</p>'));
        });

        group('insert and delete', () {
          test('prepend', () {
            final editor = createEditor('<p>0123</p>');
            editor.applyDelta(Delta()
              ..insert({'video': '#'})
              ..delete(2));
            expect(editor.scroll.element, EqualHTML('$_video<p>23</p>'));
          });

          test('insert to the middle of text', () {
            final editor = createEditor('<p>abc</p>');
            editor.applyDelta(Delta()
              ..retain(1)
              ..insert({'video': '#'})
              ..delete(2));
            expect(
                editor.scroll.element, EqualHTML('<p>a</p>$_video<p><br></p>'));
          });

          test('insert after \\n', () {
            final editor = createEditor('<p>a</p><p>cda</p>');
            editor.applyDelta(Delta()
              ..retain(2)
              ..insert({'video': '#'})
              ..delete(2));
            expect(editor.scroll.element, EqualHTML('<p>a</p>$_video<p>a</p>'));
          });

          test('insert after an inline embed', () {
            final editor = createEditor(
                '<p><img src="/assets/favicon.png"></p><p>abc</p>');
            editor.applyDelta(Delta()
              ..retain(1)
              ..insert({'video': '#'})
              ..delete(2));
            expect(
              editor.scroll.element,
              EqualHTML(
                  '<p><img src="/assets/favicon.png"></p>$_video<p>bc</p>'),
            );
          });

          test('insert after a block embed', () {
            final editor = createEditor('$_video<p>abc</p>');
            editor.applyDelta(Delta()
              ..retain(1)
              ..insert({'video': '#'})
              ..delete(2));
            expect(editor.scroll.element, EqualHTML('$_video$_video<p>c</p>'));
          });
        });

        test('append formatted block embed', () {
          final editor = createEditor('<p>0123</p><p><br></p>');
          editor.applyDelta(Delta()
            ..retain(5)
            ..insert({'video': '#'}, {'align': 'right'}));
          expect(
            editor.scroll.element,
            EqualHTML(
              '<p>0123</p><iframe src="#" class="ql-video ql-align-right" '
              'frameborder="0" allowfullscreen="true"></iframe><p><br></p>',
            ),
          );
        });
      });

      test('append', () {
        final editor = createEditor('<p>0123</p>');
        editor.applyDelta(Delta()
          ..retain(5)
          ..insert('5678'));
        expect(editor.scroll.element, EqualHTML('<p>0123</p><p>5678</p>'));
      });

      test('append newline', () {
        final editor = createEditor('<p>0123</p>');
        editor.applyDelta(Delta()
          ..retain(5)
          ..insert('\n', {'header': 2}));
        expect(editor.scroll.element, EqualHTML('<p>0123</p><h2><br></h2>'));
      });

      test('append text with newline', () {
        final editor = createEditor('<p>0123</p>');
        editor.applyDelta(Delta()
          ..retain(5)
          ..insert('5678')
          ..insert('\n', {'header': 2}));
        expect(editor.scroll.element, EqualHTML('<p>0123</p><h2>5678</h2>'));
      });

      test('append non-isolated newline', () {
        final editor = createEditor('<p>0123</p>');
        editor.applyDelta(Delta()
          ..retain(5)
          ..insert('5678\n', {'header': 2}));
        expect(editor.scroll.element, EqualHTML('<p>0123</p><h2>5678</h2>'));
      });

      test('eventual append', () {
        final editor = createEditor('<p>0123</p>');
        editor.applyDelta(Delta()
          ..retain(2)
          ..insert('ab\n', {'header': 1})
          ..retain(3)
          ..insert('cd\n', {'header': 2}));
        expect(editor.scroll.element,
            EqualHTML('<h1>01ab</h1><p>23</p><h2>cd</h2>'));
      });

      test('append text, embed and newline', () {
        final editor = createEditor('<p>0123</p>');
        editor.applyDelta(Delta()
          ..retain(5)
          ..insert('5678')
          ..insert({'image': '/assets/favicon.png'})
          ..insert('\n', {'header': 2}));
        expect(
          editor.scroll.element,
          EqualHTML('<p>0123</p><h2>5678<img src="/assets/favicon.png"></h2>'),
        );
      });

      test('append multiple lines', () {
        final editor = createEditor('<p>0123</p>');
        editor.applyDelta(Delta()
          ..retain(5)
          ..insert('56')
          ..insert('\n', {'header': 1})
          ..insert('89')
          ..insert('\n', {'header': 2}));
        expect(editor.scroll.element,
            EqualHTML('<p>0123</p><h1>56</h1><h2>89</h2>'));
      });

      test('code block', () {
        final editor = createEditor(
          '<p>0</p><div class="ql-code-block-container">'
          '<div class="ql-code-block">1</div>'
          '<div class="ql-code-block">23</div></div><p><br></p>',
        );
        editor.applyDelta(Delta()
          ..delete(4)
          ..retain(1)
          ..delete(2));
        expect(editor.scroll.element.innerHTML, '<p>2</p>');
      });

      test('prepending bold with a newline and unformatted text', () {
        final editor = createEditor('<p><strong>a</strong></p>');
        editor.applyDelta(Delta()..insert('\n1'));
        expect(editor.scroll.element,
            EqualHTML('<p><br></p><p>1<strong>a</strong></p>'));
      });
    });

    group('insertContents', () {
      test('ignores empty delta', () {
        final editor = createEditor('<p>1</p>');
        editor.insertContents(0, Delta());
        expectDelta(editor.getDelta(), Delta()..insert('1\n'));

        editor.insertContents(0, Delta()..retain(100));
        expectDelta(editor.getDelta(), Delta()..insert('1\n'));
      });

      test('prepend to paragraph', () {
        final editor = createEditor('<p>2</p>');
        editor.insertContents(0, Delta()..insert('1'));
        expectDelta(editor.getDelta(), Delta()..insert('12\n'));

        editor.insertContents(
          0,
          Delta()
            ..insert('a', {'bold': true})
            ..insert('\n', {'header': 1})
            ..insert('b', {'bold': true}),
        );

        expectDelta(
          editor.getDelta(),
          Delta()
            ..insert('a', {'bold': true})
            ..insert('\n', {'header': 1})
            ..insert('b', {'bold': true})
            ..insert('12\n'),
        );
      });

      test('prepend to list item', () {
        final editor = createEditor('<ol><li data-list="bullet">2</li></ol>');
        editor.insertContents(0, Delta()..insert('1'));
        expectDelta(
          editor.getDelta(),
          Delta()
            ..insert('12')
            ..insert('\n', {'list': 'bullet'}),
        );

        editor.insertContents(
          0,
          Delta()
            ..insert('a', {'bold': true})
            ..insert('\n', {'header': 1})
            ..insert('b', {'bold': true}),
        );

        expectDelta(
          editor.getDelta(),
          Delta()
            ..insert('a', {'bold': true})
            ..insert('\n', {'header': 1})
            ..insert('b', {'bold': true})
            ..insert('12')
            ..insert('\n', {'list': 'bullet'}),
        );
      });

      test('insert before formatting', () {
        final registry = registryWithFormats([]);
        registry.register(RegistryEntry(
          blotName: 'my-blot',
          scope: Scope.BLOCK_BLOT,
          tagNames: const ['DIV'],
          classNames: const ['my-blot'],
          create: ([dynamic value]) {
            if (value is DomElement) return _MyBlot(value);
            final node = testAdapter.document.createElement('div');
            node.classes.add('my-blot');
            return _MyBlot(node);
          },
        ));
        final root = testAdapter.document.createElement('div');
        testAdapter.document.body.append(root);
        final editor = Editor(Scroll(registry, root, emitter: Emitter()));

        editor.insertContents(
          0,
          Delta()
            ..insert('\n')
            ..insert('hi')
            ..insert('\n', {
              'my-blot': true,
              'test-style': 'random',
            }),
        );

        expect(
            editor.scroll.element.innerHTML, contains('test-style="random"'));
      });

      group('prepend to block embed', () {
        test('without ending with \\n', () {
          final editor = createEditor(_video);
          editor.insertContents(0, Delta()..insert('a'));
          expectDelta(
            editor.getDelta(),
            Delta()
              ..insert('a\n')
              ..insert({'video': '#'}),
          );
        });

        test('empty first line', () {
          final editor = createEditor('<p></p>$_video');
          editor.insertContents(1, Delta()..insert('\nworld\n'));
          expectDelta(
            editor.getDelta(),
            Delta()
              ..insert('\n\nworld\n')
              ..insert({'video': '#'}),
          );
        });

        test('multiple lines', () {
          final editor = createEditor(_video);
          editor.insertContents(
            0,
            Delta()
              ..insert('a')
              ..insert('\n', {'header': 1}),
          );
          expectDelta(
            editor.getDelta(),
            Delta()
              ..insert('a')
              ..insert('\n', {'header': 1})
              ..insert({'video': '#'}),
          );
        });
      });

      group('append', () {
        test('appends to editor', () {
          final editor = createEditor('<p>1</p>');
          editor.insertContents(2, Delta()..insert('a'));
          expectDelta(editor.getDelta(), Delta()..insert('1\na\n'));
          editor.insertContents(
            4,
            Delta()
              ..insert('b')
              ..insert('\n', {'header': 1}),
          );
          expectDelta(
            editor.getDelta(),
            Delta()
              ..insert('1\na\nb')
              ..insert('\n', {'header': 1}),
          );
        });

        test('appends to paragraph', () {
          final editor = createEditor('<p>1</p><p>2</p>');
          editor.insertContents(2, Delta()..insert('a'));
          expectDelta(editor.getDelta(), Delta()..insert('1\na2\n'));
          editor.insertContents(
            2,
            Delta()
              ..insert('b')
              ..insert('\n', {'header': 1}),
          );
          expectDelta(
            editor.getDelta(),
            Delta()
              ..insert('1\nb')
              ..insert('\n', {'header': 1})
              ..insert('a2\n'),
          );
        });

        test('appends to block embed', () {
          final editor = createEditor('$_video<p>2</p>');
          editor.insertContents(1, Delta()..insert('1'));
          expectDelta(
            editor.getDelta(),
            Delta()
              ..insert({'video': '#'})
              ..insert('12\n'),
          );
          editor.insertContents(
            1,
            Delta()
              ..insert('b')
              ..insert('\n', {'header': 1}),
          );
          expectDelta(
            editor.getDelta(),
            Delta()
              ..insert({'video': '#'})
              ..insert('b')
              ..insert('\n', {'header': 1})
              ..insert('12\n'),
          );
        });
      });

      test('prepends a formatted block embed', () {
        final editor = createEditor('<p></p>');
        editor.insertContents(
          0,
          Delta()..insert({'video': '#'}, {'width': '300'}),
        );
        expectDelta(
          editor.getDelta(),
          Delta()
            ..insert({'video': '#'}, {'width': '300'})
            ..insert('\n'),
        );
      });

      test('prepends two formatted block embeds', () {
        final editor = createEditor('<p></p>');
        editor.insertContents(
          0,
          Delta()
            ..insert({'video': '#'}, {'width': '300'})
            ..insert({'video': '#'}, {'width': '600'}),
        );
        expectDelta(
          editor.getDelta(),
          Delta()
            ..insert({'video': '#'}, {'width': '300'})
            ..insert({'video': '#'}, {'width': '600'})
            ..insert('\n'),
        );
      });

      test('inserts formatted block embeds (styles)', () {
        final editor = createEditor('<p></p>');
        editor.insertContents(
          0,
          Delta()
            ..insert('a\n')
            ..insert({'video': '#'}, {'width': '300'})
            ..insert({'video': '#'}, {'width': '300'})
            ..insert('\nd'),
        );
        expectDelta(
          editor.getDelta(),
          Delta()
            ..insert('a\n')
            ..insert({'video': '#'}, {'width': '300'})
            ..insert({'video': '#'}, {'width': '300'})
            ..insert('\nd\n'),
        );
      });

      test('inserts formatted block embeds (attributor)', () {
        final editor = createEditor('<p></p>');
        editor.insertContents(
          0,
          Delta()
            ..insert('a\n')
            ..insert({'video': '#'}, {'align': 'center'})
            ..insert({'video': '#'}, {'align': 'center'})
            ..insert('\nd'),
        );
        expectDelta(
          editor.getDelta(),
          Delta()
            ..insert('a\n')
            ..insert({'video': '#'}, {'align': 'center'})
            ..insert({'video': '#'}, {'align': 'center'})
            ..insert('\nd\n'),
        );
      });

      test('inserts inline embeds to bold text', () {
        final editor = createEditor('<p><strong>ab</strong></p>');
        editor.insertContents(1, Delta()..insert({'image': '#'}));
        expectDelta(
          editor.getDelta(),
          Delta()
            ..insert('a', {'bold': true})
            ..insert({'image': '#'})
            ..insert('b', {'bold': true})
            ..insert('\n'),
        );
      });

      test('inserts multiple lines to a container', () {
        final editor = createEditor('<ol><li data-list="ordered"></li></ol>');
        editor.insertContents(
          0,
          Delta()
            ..insert('world', {'font': 'monospace'})
            ..insert('\n', {'list': 'bullet'})
            ..insert('\n'),
        );
        expectDelta(
          editor.getDelta(),
          Delta()
            ..insert('world', {'font': 'monospace'})
            ..insert('\n', {'list': 'bullet'})
            ..insert('\n')
            ..insert('\n', {'list': 'ordered'}),
        );
      });

      group('invalid delta', () {
        Delta editorDelta(void Function(Editor editor) modify) {
          final editor = createEditor('<p></p>');
          modify(editor);
          return editor.getDelta();
        }

        test('conflict block formats', () {
          final change = Delta()
            ..insert('a')
            ..insert('\n', {'header': 1, 'list': 'bullet'})
            ..insert('b')
            ..insert('\n', {'header': 1, 'list': 'bullet'});

          expectDelta(
            editorDelta((editor) => editor.insertContents(0, change)),
            editorDelta((editor) => editor.applyDelta(change)),
          );
        });

        test('block embeds with line formats', () {
          final change = Delta()
            ..insert('a\n')
            ..insert({'video': '#'}, {'header': 1})
            ..insert({'video': '#'}, {'header': 1})
            ..insert('\n', {'header': 1});

          expectDelta(
            editorDelta((editor) => editor.insertContents(0, change)),
            editorDelta((editor) => editor.applyDelta(change)),
          );
        });

        test('missing \\n before block embeds', () {
          final change = Delta()
            ..insert('a')
            ..insert({'video': '#'})
            ..insert('b\n');

          expectDelta(
            editorDelta((editor) => editor.insertContents(0, change)),
            editorDelta((editor) => editor.applyDelta(change)),
          );
        });
      });
    });

    group('getFormat()', () {
      test('unformatted', () {
        final editor = createEditor('<p>0123</p>');
        expect(editor.getFormat(1), <String, dynamic>{});
      });

      test('formatted', () {
        final editor = createEditor('<h1><em>0123</em></h1>');
        expect(editor.getFormat(1), {'header': 1, 'italic': true});
      });

      test('cursor', () {
        final editor = createEditor(
          '<h1><strong><em>0123</em></strong></h1><h2><u>5678</u></h2>',
        );
        expect(
            editor.getFormat(2), {'bold': true, 'italic': true, 'header': 1});
      });

      test('cursor with preformat', () {
        final quill =
            createEditorQuill('<h1><strong><em>0123</em></strong></h1>');
        quill.selection.setRange(const Range(2));
        quill.selection.format('underline', true);
        quill.selection.format('color', 'red');
        expect(quill.editor.getFormat(2), {
          'bold': true,
          'italic': true,
          'header': 1,
          'color': 'red',
          'underline': true,
        });
      });

      test('across leaves', () {
        final editor = createEditor('''
          <h1>
            <strong class="ql-size-small"><em>01</em></strong>
            <em class="ql-size-large"><u>23</u></em>
            <em class="ql-size-huge"><u>45</u></em>
          </h1>
        ''');
        expect(editor.getFormat(1, 4), {
          'italic': true,
          'header': 1,
          'size': ['small', 'large', 'huge'],
        });
      });

      test('across leaves repeated', () {
        final editor = createEditor('''
          <h1>
            <strong class="ql-size-small"><em>01</em></strong>
            <em class="ql-size-large"><u>23</u></em>
            <em class="ql-size-huge"><u>45</u></em>
            <em class="ql-size-small"><u>45</u></em>
          </h1>
        ''');
        expect(editor.getFormat(1, 4), {
          'italic': true,
          'header': 1,
          'size': ['small', 'large', 'huge'],
        });
      });

      test('across lines repeated', () {
        final editor = createEditor('''
          <h1 class="ql-align-right"><em>01</em></h1>
          <h1 class="ql-align-center"><em>34</em></h1>
          <h1 class="ql-align-right"><em>36</em></h1>
          <h1 class="ql-align-center"><em>33</em></h1>
        ''');
        expect(editor.getFormat(1, 3), {
          'italic': true,
          'header': 1,
          'align': ['right', 'center'],
        });
      });

      test('across lines', () {
        final editor = createEditor('''
          <h1 class="ql-align-right"><em>01</em></h1>
          <h1 class="ql-align-center"><em>34</em></h1>
        ''');
        expect(editor.getFormat(1, 3), {
          'italic': true,
          'header': 1,
          'align': ['right', 'center'],
        });
      });
    });

    group('getHTML', () {
      test('inline', () {
        expect(
            createEditor('<blockquote>Test</blockquote>').getHTML(1, 2), 'es');
        expect(createEditor('<blockquote>Test</blockquote>').getHTML(0, 4),
            'Test');
      });

      test('entire line', () {
        final editor = createEditor('<blockquote>Test</blockquote>');
        expect(editor.getHTML(0, 5), '<blockquote>Test</blockquote>');
      });

      test('entire list item', () {
        final editor = createEditor('<ul><li>Test</li></ul>');
        expect(editor.getHTML(0, 5), '<ul><li>Test</li></ul>');
      });

      test('across lines', () {
        final editor = createEditor(
          '<h1 class="ql-align-center">Header</h1><p>Text</p>'
          '<blockquote>Quote</blockquote>',
        );
        expect(
          editor.getHTML(1, 14),
          '<h1 class="ql-align-center">eader</h1><p>Text</p>'
          '<blockquote>Quo</blockquote>',
        );
      });

      test('collapsible spaces', () {
        expect(
          createEditor('<p><strong>123 </strong>123<em> 123</em></p>')
              .getHTML(0, 11),
          '<strong>123&nbsp;</strong>123<em>&nbsp;123</em>',
        );

        expect(
          createEditorFromDelta(Delta()..insert('1   2\n')).getHTML(0, 5),
          '1&nbsp;&nbsp;&nbsp;2',
        );

        expect(
          createEditorFromDelta(Delta()
                ..insert('  123', {'bold': true})
                ..insert('\n'))
              .getHTML(0, 5),
          '<strong>&nbsp;&nbsp;123</strong>',
        );
      });

      test('mixed list', () {
        final editor = createEditor('''
          <ol>
            <li data-list="ordered">One</li>
            <li data-list="ordered">Two</li>
            <li data-list="bullet">Foo</li>
            <li data-list="bullet">Bar</li>
          </ol>
        ''');
        expectHtmlStringEquals(editor.getHTML(2, 12), '''
          <ol>
            <li>e</li>
            <li>Two</li>
          </ol>
          <ul>
            <li>Foo</li>
            <li>Ba</li>
          </ul>
        ''');
      });

      test('nested list', () {
        final editor = createEditor('''
          <ol>
            <li data-list="ordered">One</li>
            <li data-list="ordered">Two</li>
            <li data-list="bullet" class="ql-indent-1">Alpha</li>
            <li data-list="ordered" class="ql-indent-2">I</li>
            <li data-list="ordered" class="ql-indent-2">II</li>
            <li data-list="ordered">Three</li>
          </ol>
        ''');
        expectHtmlStringEquals(editor.getHTML(2, 20), '''
          <ol>
            <li>e</li>
            <li>Two
              <ul>
                <li>Alpha
                  <ol>
                    <li>I</li>
                    <li>II</li>
                  </ol>
                </li>
              </ul>
            </li>
            <li>Thr</li>
          </ol>
        ''');
      });

      test('nested checklist', () {
        final editor = createEditor('''
          <ol>
            <li data-list="checked">One</li>
            <li data-list="checked">Two</li>
            <li data-list="unchecked" class="ql-indent-1">Alpha</li>
            <li data-list="checked" class="ql-indent-2">I</li>
            <li data-list="checked" class="ql-indent-2">II</li>
            <li data-list="checked">Three</li>
          </ol>
        ''');
        expectHtmlStringEquals(editor.getHTML(2, 20), '''
          <ul>
            <li data-list="checked">e</li>
            <li data-list="checked">Two
              <ul>
                <li data-list="unchecked">Alpha
                  <ul>
                    <li data-list="checked">I</li>
                    <li data-list="checked">II</li>
                  </ul>
                </li>
              </ul>
            </li>
            <li data-list="checked">Thr</li>
          </ul>
        ''');
      });

      test('partial list', () {
        final editor = createEditor('''
          <ol>
            <li data-list="ordered">1111</li>
            <li data-list="ordered" class="ql-indent-1">AAAA</li>
            <li data-list="ordered" class="ql-indent-2">IIII</li>
            <li data-list="ordered" class="ql-indent-1">BBBB</li>
            <li data-list="ordered">2222</li>
          </ol>
        ''');
        expectHtmlStringEquals(editor.getHTML(12, 12), '''
          <ol>
            <li>
              <ol>
                <li>
                  <ol>
                    <li>II</li>
                  </ol>
                </li>
                <li>BBBB</li>
              </ol>
            </li>
            <li>2222</li>
          </ol>
        ''');
      });

      test('text within tag', () {
        final editor = createEditor('<p><strong>a</strong></p>');
        expect(editor.getHTML(0, 1), '<strong>a</strong>');
      });

      test('escape html', () {
        final editor = createEditor('<p><br></p>');
        editor.insertText(0, '<b>Test</b>');
        expect(editor.getHTML(0, 11), '&lt;b&gt;Test&lt;/b&gt;');
      });

      test('multiline code', () {
        final editor = createEditor(
          '<p><br></p><p>0123</p><p><br></p><p><br></p><p>4567</p>'
          '<p><br></p>',
        );
        final length = editor.scroll.length();
        editor.formatLine(0, length, {'code-block': 'javascript'});

        // Upstream's `createEditor` builds a CORE registry (`CodeBlock` +
        // `CodeBlockContainer`), so it expects a bare `<pre>`. This port's
        // default registry is the FULL build: registering `modules/syntax`
        // runs `Syntax.register()`, which overwrites `code-block` with the
        // syntax blots whose `html()` emits `data-language` — the same output
        // the real quill.js bundle produces, recorded in the G10 goldens.
        const open = '<pre data-language="javascript">';
        expect(editor.getHTML(0, length), '$open\n\n0123\n\n\n4567\n\n</pre>');
        expect(editor.getHTML(1, 7), '$open\n0123\n\n\n\n</pre>');
        expect(editor.getHTML(2, 7), '$open\n123\n\n\n4\n</pre>');
        expect(editor.getHTML(5, 7), '$open\n\n\n\n4567\n</pre>');
      });
    });
  });
}
