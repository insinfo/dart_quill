...
NodeRange? range(int pos, [int? end]) {
  return testDoc
      .resolve(pos)
      .blockRange(end == null ? null : testDoc.resolve(end));
}
...
void main() {
  group('canSplit', () {
    void Function() yes(int pos, [int depth = 1, String? after]) {
      return () => expect(
          canSplit(testDoc, pos, depth,
              after == null ? null : [Wrapping(schema.nodes[after]!)]),
          isTrue);
    }
    ...
    test(
        'doesn\'t return true when the split-off content doesn\'t fit in the given node type',
        () {
          ...
          canSplit(
              s.node("doc", null, [
                s.node("chapter", null, [
                  ...
                  s.node("scene", null, [
                    s.node("para", null, [s.text("scene")])
                  ])
                ])
              ]),
              4,
              1,
              [Wrapping(s.nodes['scene']!)]),
          isFalse);
    });
  });
  ...
  group('liftTarget', () {
    ...
    test('notices unliftable content after or before', () {
      final s = Schema(SchemaSpec(nodes: {
      ...
      final d = s.node("doc", null, [
        s.node("section", null, [
          s.node("heading", null, [pNode, pNode, pNode]),
          pNode
        ])
      ]);
  ...
  group('findWrapping', () {
    ...
    void Function() no(int pos, int end, String type) {
      return () {
        final r = range(pos, end);
        expect(
            r == null || findWrapping(r, schema.nodes[type]!) == null, isTrue);
      };
    }
  ...
  group('Transform', () {
    group('replace', () {
      void Function() repl(PMNode docNode, int from, int to, PMNode? content,
          int openStart, int openEnd, PMNode result) {
        return () {
