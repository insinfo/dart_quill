...
int getTag(PMNode node, String tagKey) {
  return node.tag[tagKey] ??
      (throw Exception("Missing tag \$tagKey on \$node"));
}
  ...
  group('Transform', () {
    group('addMark', () {
      void add(PMNode docNode, Mark mark, PMNode expectNode) {
        testTransform(
            Transform(docNode)
                .addMark(getTag(docNode, "a"), getTag(docNode, "b"), mark),
            expectNode);
      }
      ...
      test(
          'should overwrite marks with different attributes',
          () => add(
              doc(p("this is a ", a({"href": "bar"}, "<a>link<b>"))),
      ...
      test(
          ...
          () => add(
              doc(p("before"), blockquote(p("the variable is called <a>i<b>")),
                  p("after")),
      ...
      test(
          ...
          () => add(
              doc(p("hi <a>this"), blockquote(p("is")), p("a docu<b>ment"),
                  p("!")),
              testSchema.marks['em']!.create(),
              doc(p("hi ", em("this")), blockquote(p(em("is"))),
                  p(em("a docu"), "ment"), p("!"))));
    });
    ...
    group('removeMark', () {
      void rem(PMNode docNode, Mark? mark, PMNode expectNode) {
        testTransform(
            Transform(docNode)
                .removeMark(getTag(docNode, "a"), getTag(docNode, "b"), mark),
            expectNode);
      }
      ...
      test(
          'doesn\'t do anything when there\'s no mark',
          () => rem(
              doc(p(em("hello"), " <a>world<b>!")),
              testSchema.marks['em']!.create(),
              doc(p(em("hello"), " <a>world<b>!"))));
      ...
      test(
          'can remove a link',
          () => rem(
              doc(p("<a>hello ", a({"href": "foo"}, "link<b>"))),
              testSchema.marks['link']!.create({'href': "foo"}),
              doc(p("hello link"))));

      test(
          'doesn\'t remove a non-matching link',
          () => rem(
              doc(p("<a>hello ", a({"href": "bar"}, "link<b>"))),
              testSchema.marks['link']!.create({'href': "foo"}),
              doc(p("hello ", a({"href": "bar"}, "link")))));

      test(
          'can remove across blocks',
          () => rem(
              doc(blockquote(p(em("much <a>em")), p(em("here too"))),
                  p("between", em("...")), p(em("end<b>"))),
              testSchema.marks['em']!.create(),
              doc(blockquote(p(em("much "), "em"), p("here too")),
                  p("between..."), p("end"))));
    ...
    group('insert', () {
      void ins(PMNode docNode, dynamic nodes, PMNode expectNode) {
        testTransform(
            Transform(docNode).insert(getTag(docNode, "a"), nodes), expectNode);
      }
      ...
      test(
          ...
          () => ins(
              doc(p("one"), "<a>", p("two<2>")),
              [
                testSchema.nodes['paragraph']!
                    .create(null, [testSchema.text("hi")]),
    ...
    group('delete', () {
      void del(PMNode docNode, PMNode expectNode) {
        testTransform(
            Transform(docNode)
                .delete(getTag(docNode, "a"), getTag(docNode, "b")),
            expectNode);
      }
      ...
      test(
          'preserves content constraints',
          () => del(doc(blockquote("<a>", p("hi"), "<b>"), p("x")),
              doc(blockquote(p()), p("x"))));

      test(
          'preserves positions after the range',
    ...
    group('join', () {
      void join(PMNode docNode, PMNode expectNode) {
        testTransform(
            Transform(docNode).join(getTag(docNode, "a")), expectNode);
      }

      test(
          'can join blocks',
          () => join(
              doc(blockquote(p("<before>a")), "<a>", blockquote(p("b")),
                  p("after<after>")),
              doc(blockquote(p("<before>a"), "<a>", p("b")),
                  p("after<after>"))));
      ...
      test(
          'can join nested blocks',
          () => join(
              doc(blockquote(blockquote(p("a"), p("b<before>")), "<a>",
                  blockquote(p("c"), p("d<after>")))),
              doc(blockquote(blockquote(
                  p("a"), p("b<before>"), "<a>", p("c"), p("d<after>"))))));
    ...
    group('split', () {
      void split(PMNode docNode, dynamic expectNode,
          [int depth = 1, List<Wrapping?>? typesAfter]) {
        if (expectNode == "fail") {
          expect(
              () => Transform(docNode)
                  .split(getTag(docNode, "a"), depth, typesAfter),
              throwsA(isA<TransformError>()));
        } else {
          testTransform(
              Transform(docNode).split(getTag(docNode, "a"), depth, typesAfter),
              expectNode as PMNode);
        }
      }

      test('can split a textblock',
          () => split(doc(p("foo<a>bar")), doc(p("foo"), p("<a>bar"))));

      test(
          'correctly maps positions',
      ...
      test(
          ...
          () => split(
              ...
              doc(blockquote(blockquote(p("foo")), blockquote(p("<a>bar"))),
                  p("after<1>")),
              2));

      test(
          'can split three deep',
          () => split(
              doc(blockquote(blockquote(p("foo<a>bar"))), p("after<1>")),
              doc(blockquote(blockquote(p("foo"))),
                  blockquote(blockquote(p("<a>bar"))), p("after<1>")),
              3));
      ...
      test(
          'can split inside a list item',
          () => split(
              doc(ol(li(p("one<1>")), li(p("two<a>three")), li(p("four<2>")))),
              doc(ol(li(p("one<1>")), li(p("two"), p("<a>three")),
                  li(p("four<2>"))))));

      test(
          'can split a list item',
          () => split(
              doc(ol(li(p("one<1>")), li(p("two<a>three")), li(p("four<2>")))),
              doc(ol(li(p("one<1>")), li(p("two")), li(p("<a>three")),
                  li(p("four<2>")))),
              2));

      test(
          'respects the type param',
          () => split(doc(h1("hell<a>o!")), doc(h1("hell"), p("<a>o!")), 1,
              [Wrapping(testSchema.nodes['paragraph']!)]));
    ...
    group('lift', () {
      void lift(PMNode docNode, PMNode expectNode) {
        NodeRange? range = docNode.resolve(getTag(docNode, "a")).blockRange(
            docNode.resolve(tagOpt(docNode, "b") ?? getTag(docNode, "a")));
        testTransform(
            Transform(docNode).lift(range!, liftTarget(range)!), expectNode);
      }
      ...
      test(
          'finds a valid range from a lopsided selection',
          () => lift(
              doc(p("start"),
                  blockquote(blockquote(p("a"), p("<a>b")), p("<b>c"))),
              doc(p("start"), blockquote(p("a"), p("<a>b")), p("<b>c"))));

      test(
          'can lift from a nested node',
          () => lift(
              doc(blockquote(blockquote(p("<1>one"), p("<a>two"), p("<3>three"),
                  p("<b>four"), p("<5>five")))),
              doc(blockquote(blockquote(p("<1>one")), p("<a>two"),
                  p("<3>three"), p("<b>four"), blockquote(p("<5>five"))))));
      ...
      test(
          'can lift from the end of a list',
          () => lift(doc(ul(li(p("a")), li(p("b<a>")), "<1>")),
              doc(ul(li(p("a"))), p("b<a>"), "<1>")));
    });

    group('wrap', () {
      void wrap(PMNode docNode, PMNode expectNode, String type,
          [Map<String, dynamic>? attrs]) {
        NodeRange? range = docNode.resolve(getTag(docNode, "a")).blockRange(
            docNode.resolve(tagOpt(docNode, "b") ?? getTag(docNode, "a")));
        testTransform(
            Transform(docNode).wrap(
                range!, findWrapping(range, testSchema.nodes[type]!, attrs)!),
            expectNode);
      }
      ...
      test(
          ...
          () => wrap(
              ...
              doc(p("one<1>"), blockquote(p("<a>two"), p("three")),
                  p("four<4>")),
              "blockquote"));
      ...
      test(
          ...
          () => wrap(doc(p("<a>one"), p("<b>two")),
              doc(ul(li(p("<a>one"), p("<b>two")))), "bullet_list"));
    });

    group('setBlockType', () {
      void set(PMNode docNode, PMNode expectNode, String type,
          [Map<String, dynamic>? attrs]) {
        testTransform(
            Transform(docNode).setBlockType(
                getTag(docNode, "a"),
                tagOpt(docNode, "b") ?? getTag(docNode, "a"),
                testSchema.nodes[type],
                attrs),
            expectNode);
      }

      test(
          'can change a single textblock',
          () =>
              set(doc(p("am<a> i")), doc(h2("am i")), "heading", {"level": 2}));

      test(
          'can change multiple blocks',
          () => set(doc(h1("<a>hello"), p("there"), p("<b>!"), p("x")),
              doc(pre("hello"), pre("there"), pre("!"), p("x")), "code_block"));

      test(
          'can change a wrapped block',
          () => set(doc(blockquote(p("<a>hi"))), doc(blockquote(h1("hi"))),
              "heading", {"level": 1}));

      test(
          'clears markup when necessary',
          () => set(doc(p("<a>v", em("ery"), " ", strong("well"))),
              doc(pre("very well")), "code_block"));
    });
    ...
    group('setNodeMarkup', () {
      void set(PMNode docNode, PMNode expectNode, String? type,
          [Map<String, dynamic>? attrs]) {
        testTransform(
            Transform(docNode).setNodeMarkup(getTag(docNode, "a"),
                type != null ? testSchema.nodes[type] : null, attrs),
            expectNode);
      }

      test(
          'can change the markup of a paragraph',
          () => set(
              doc("<a>", p("foo")), doc(h1("foo")), "heading", {"level": 1}));

      test(
          'can change the markup of an inline node',
          () => set(
              doc(p("foo<a>", img({"src": "foo.png"}), "bar")),
              doc(p("foo", img({"alt": "x", "src": "foo.png"}), "bar")),
              null,
              {"alt": "x", "src": "foo.png"}));
    });
    ...
    group('setNodeAttribute', () {
      void set(PMNode docNode, String attr, dynamic value, PMNode expectNode) {
        testTransform(
            Transform(docNode)
                .setNodeAttribute(getTag(docNode, "a"), attr, value),
            expectNode);
      }
