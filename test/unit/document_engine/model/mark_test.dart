...
void main() {
  ...
  Mark link(String href, [String? title]) =>
      testSchema.marks['link']!.create({'href': href, 'title': title});
  final code = testSchema.marks['code']!.create();

  final customSchema = Schema(SchemaSpec(nodes: {
    'doc': NodeSpec(content: 'paragraph+'),
    'paragraph': NodeSpec(content: 'text*'),
    'text': NodeSpec()
  }, marks: {
    'remark': MarkSpec(
        attrs: {'id': AttributeSpec()}, excludes: "", inclusive: false),
  ...
  group('Mark', () {
    group('sameSet', () {
      ...
      test(
          ...
          () => expect(
              Mark.sameSet([em_, strong], [em_, strong, code]), isFalse));

      test(
          'recognizes identical links in set',
          () => expect(
              Mark.sameSet(
                  [link("http://foo"), code], [link("http://foo"), code]),
              isTrue));
      ...
      test(
          ...
          () => expect(
              Mark.sameSet(
                  [link("http://foo"), code], [link("http://bar"), code]),
              isFalse));
    });
    ...
    group('eq', () {
      ...
      test(
          ...
          () => expect(
              link("http://foo", "A").eq(link("http://foo", "B")), isFalse));
    });
    ...
    group('addToSet', () {
      ...
      test(
          ...
          () => expect(
              Mark.sameSet(em_.addToSet([strong]), [em_, strong]), isTrue));

      test(
          'adds marks with higher rank after others',
          () => expect(
              Mark.sameSet(strong.addToSet([em_]), [em_, strong]), isTrue));

      test(
          'replaces different marks with new attributes',
          () => expect(
              Mark.sameSet(
                  link("http://bar").addToSet([link("http://foo"), em_]),
                  [link("http://bar"), em_]),
              isTrue));
      ...
      test(
          ...
          () => expect(
              Mark.sameSet(
                  link("http://foo").addToSet([em_, link("http://foo")]),
                  [em_, link("http://foo")]),
              isTrue));
      ...
      test(
          ...
          () => expect(
              Mark.sameSet(code.addToSet([em_, strong, link("http://foo")]),
      ...
      test(
          ...
          () => expect(
              Mark.sameSet(strong.addToSet([em_, code]), [em_, strong, code]),
              isTrue));

      test(
          'allows nonexclusive instances of marks with the same type',
          () => expect(
              Mark.sameSet(remark2.addToSet([remark1]), [remark1, remark2]),
              isTrue));

      test(
          'doesn\'t duplicate identical instances of nonexclusive marks',
          () => expect(
              Mark.sameSet(remark1.addToSet([remark1]), [remark1]), isTrue));

      test(
          'clears all others when adding a globally-excluding mark',
          () => expect(
              Mark.sameSet(user1.addToSet([remark1, customEm]), [user1]),
              isTrue));

      test(
          'does not allow adding another mark to a globally-excluding mark',
          () => expect(
              Mark.sameSet(customEm.addToSet([user1]), [user1]), isTrue));
      ...
      test(
          ...
          () => expect(
              Mark.sameSet(customEm.addToSet([remark1, customStrong]),
                  [remark1, customStrong]),
              isTrue));

      test(
          'remove excluded marks when adding a mark',
          () => expect(
              Mark.sameSet(customStrong.addToSet([remark1, customEm]),
                  [remark1, customStrong]),
              isTrue));
    });
    ...
    group('removeFromSet', () {
      ...
      test(
          ...
          () => expect(
              Mark.sameSet(
                  link("http://foo").removeFromSet([link("http://foo")]), []),
              isTrue));
    ...
    group('ResolvedPos.marks', () {
      void isAt(PMNode docNode, Mark mark, bool result) {
        expect(mark.isInSet(docNode.resolve(docNode.tag['a']!).marks()),
            equals(result));
      }
      ...
      test(
          ...
          () => isAt(doc(p(a({'href': 'foo'}, "li<a>nk"))), link("http://baz"),
              false));

      final customDoc = customSchema.node("doc", null, [
        customSchema.node("paragraph", null, [
          // pos 1
          customSchema.text("one", [remark1, customStrong]),
          customSchema.text("two")
        ]),
        customSchema.node("paragraph", null, [
          // pos 9
          customSchema.text("one"), customSchema.text("two", [remark1]),
          customSchema.text("three", [remark1])
        ]), // pos 22
        customSchema.node("paragraph", null, [
          customSchema.text("one", [remark2]),
          customSchema.text("two", [remark1])
        ])
      ]);
      ...
      test(
          ...
          () => expect(
              Mark.sameSet(customDoc.resolve(4).marks(), [customStrong]),
              isTrue));
      ...
      test(
          ...
          () => expect(
              Mark.sameSet(
                  customDoc.resolve(3).marks(), [remark1, customStrong]),
              isTrue));
      ...
      test(
          ...
          () => expect(
              Mark.sameSet(customDoc.resolve(15).marks(), [remark1]), isTrue));
