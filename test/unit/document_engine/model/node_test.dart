...
void main() {
  final customSchema = Schema(SchemaSpec(nodes: {
    ...
    'text':
        NodeSpec(), // Emulating custom debug string would require dart changes to PMNode, skip
    'contact': NodeSpec(
        inline: true,
        attrs: {'name': AttributeSpec(), 'email': AttributeSpec()},
        leafText: (PMNode node) =>
  ...
  group('Node', () {
    ...
    group('cut', () {
      ...
      test(
          ...
          () => cut(
              doc(p("foo"), "<a>", p("bar"), "<b>", p("baz")), doc(p("bar"))));

      test('cuts text',
          () => cut(doc(p("0"), p("foo<a>bar<b>baz"), p("2")), doc(p("bar"))));
      ...
      test('preserves marks', () {
        PMNode d = doc(p(
            "foo",
            em("ba<a>r", img({'src': ''}), strong("baz"), br()),
            "qu<b>ux",
            code("xyz")));
    ...
    group('between', () {
      void between(PMNode docNode, List<String> nodes) {
        ...
        docNode.nodesBetween(a, b,
            (PMNode node, int pos, PMNode? parent, int index) {
          if (i == nodes.length) {
          ...
          if (!node.isText && docNode.nodeAt(pos) != node) {
            fail(
                "Pos \$pos does not point at node \$node \${docNode.nodeAt(pos)}");
          }
      ...
      test('iterates over text',
          () => between(doc(p("foo<a>bar<b>baz")), ["paragraph", "foobarbaz"]));
    ...
    group('textBetween', () {
      test('works when passing a custom function as leafText', () {
        ...
        expect(
            d.textBetween(0, d.content.size, blockSeparator: '',
                leafText: (node) {
      ...
      test('works with leafText', () {
        final d = customSchema.nodes['doc']!.createChecked({}, [
          customSchema.nodes['paragraph']!.createChecked({}, [
            ...
            customSchema.nodes['contact']!
                .createChecked({'name': "Alice", 'email': "alice@example.com"})
          ])
        ]);
        expect(d.textBetween(0, d.content.size),
            equals('Hello Alice <alice@example.com>'));
      });
      ...
      test('should ignore leafText when passing a custom leafText', () {
        ...
        expect(
            d.textBetween(0, d.content.size,
                blockSeparator: '', leafText: (PMNode node) => '<anonymous>'),
            equals('Hello <anonymous>'));
      });

      test('adds block separator around empty paragraphs', () {
        expect(
            doc(p("one"), p(), p("two"))
                .textBetween(0, 12, blockSeparator: "\n"),
            equals("one\n\ntwo"));
      });
    ...
    group('textContent', () {
      ...
      test('works on a nested element', () {
        expect(doc(ul(li(p("hi")), li(p(em("a"), "b")))).textContent,
            equals("hiab"));
      });
    });
  });
