    ...
    group('replace', () {
      ...
      test(
          ...
          () => rpl(doc(p("on<a>e"), p("t<b>wo")),
              doc(p("xx<a>xx"), p("yy<b>yy")), doc(p("onxx"), p("yywo"))));

      test(
          'merges when adding text',
          () => rpl(doc(p("on<a>e"), p("t<b>wo")), doc(p("<a>H<b>")),
              doc(p("onHwo"))));

      test(
          'can insert text',
          () => rpl(doc(p("before"), p("on<a><b>e"), p("after")),
              doc(p("<a>H<b>")), doc(p("before"), p("onHe"), p("after"))));

      test(
          'doesn\'t merge non-matching blocks',
          () => rpl(doc(p("on<a>e"), p("t<b>wo")), doc(h1("<a>H<b>")),
              doc(p("onHwo"))));

      test(
          'can merge a nested node',
          () => rpl(doc(blockquote(blockquote(p("on<a>e"), p("t<b>wo")))),
              doc(p("<a>H<b>")), doc(blockquote(blockquote(p("onHwo"))))));

      test(
          'can replace within a block',
          () => rpl(doc(blockquote(p("a<a>bc<b>d"))), doc(p("x<a>y<b>z")),
      ...
      test(
          ...
          () => rpl(
              doc(blockquote(
                  blockquote(p("on<a>e"), p("two"), "<b>", p("three")))),
              doc(blockquote(p("aa<a>aa"), p("bb"), p("cc"), "<b>", p("dd"))),
              doc(blockquote(
                  blockquote(p("onaa"), p("bb"), p("cc"), p("three"))))));

      test(
          'can insert a deep, lopsided slice',
          () => rpl(
              doc(blockquote(blockquote(p("on<a>e"), p("two"), p("three")),
                  "<b>", p("x"))),
              doc(blockquote(p("aa<a>aa"), p("bb"), p("cc")), "<b>", p("dd")),
              doc(blockquote(
                  blockquote(p("onaa"), p("bb"), p("cc")), p("x")))));

      test(
          'can merge multiple levels',
          () => rpl(
              doc(blockquote(blockquote(p("hell<a>o"))),
      ...
      test(
          'can insert a split',
          () => rpl(doc(p("foo<a><b>bar")), doc(p("<a>x"), p("y<b>")),
              doc(p("foox"), p("ybar"))));
      ...
      test(
          ...
          () => rpl(doc(h1("foo<a>bar"), "<b>"), doc(p("foo<a>baz"), "<b>"),
              doc(h1("foobaz"))));

      test(
          'keeps the node type even when empty',
          () => rpl(doc(h1("<a>bar"), "<b>"), doc(p("foo<a>baz"), "<b>"),
              doc(h1("baz"))));
      ...
      void bad(PMNode docNode, PMNode? insert, String pattern) {
        ...
        expect(
            () => docNode.replace(docNode.tag['a']!, docNode.tag['b']!, slice),
            throwsA(predicate((e) {
          return e is ReplaceError &&
              RegExp(pattern, caseSensitive: false).hasMatch(e.toString());
        })));
      }
      ...
      test(
          ...
          () => bad(
              doc(p("<a><b>")), doc(blockquote(p("<a>")), "<b>"), "deeper"));
      ...
      test(
          ...
          () => bad(doc(ul(li(p("a")), "<a>"), "<b>"),
              doc(p("foo", "<a>"), "<b>"), "cannot join"));

      test(
          'rejects an unjoinable delete',
          () => bad(doc(blockquote(p("a"), "<a>"), ul("<b>", li(p("b")))), null,
              "cannot join"));

      test(
          'check content validity',
          () => bad(doc(blockquote("<a>", p("hi")), "<b>"),
              doc(blockquote("hi", "<a>"), "<b>"), "invalid content"));
    });
  });
}
